...
export function createDialogueWriter(genre) {
  // object mein functions bhi store kar sakte hain
  const scripts = {
    action: (hero, villain) =>
  ...
  if (!scripts[genre]) return null;
  return (hero, villain) => {
    if (!hero || !villain || typeof hero !== "string" || typeof villain !== "string") {
      return "...";
    }

    return scripts[genre](hero, villain);
  };
}
  ...
  return (scores) => {
    ...
    for (const key in weights) {
      if (scores[key] !== undefined) {
        total = total + scores[key] * weights[key];
      }
    }
