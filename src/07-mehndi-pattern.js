...
export function flattenArray(arr) {
  ...
  // I got a stack overflow issue here because I did not include a base case 
  const first = arr[0];
  const rest = arr.slice(1);
...
export function generatePattern(n) {
   if (!Number.isInteger(n) || n <= 0) return [];
     function mehndiPattern(i, n) {
    if (i > n) return [];
    const current = repeatChar("*", i);
    return [current].concat(mehndiPattern(i + 1, n)).concat(i === n ? [] : [current]);
  }
   return mehndiPattern(1, n);
}
